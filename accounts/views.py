...
class ProfileView(LoginRequiredMixin, View):
    def get(self, request, pk, *args, **kwargs):
        ...
        videos = video.objects.filter(author = user).order_by('-published')
...
class ProfileEditView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    model = UserProfile
    fields = [ 'bio', 'birth_date' , "mobile" , 'relationship', 'school_name' , 'academicbackground' , 'country' , 'state', 'region', 'address' , 'address_line_2' , 'picture']
...
class Followers(LoginRequiredMixin, ListView):
    def get (self, request , pk , *args , **kwargs) :
        ...
        return render(request, 'accounts/mates_list.html' , context)
		...
		if password_reset_form.is_valid():
			...
			if associated_users.exists():
				for user in associated_users:
					...
					c = {
					"email":user.email,
					'domain':'leafnotion.com',
					'site_name': 'Leafnotion',
